...
export function FinancialWealthSection({
  ...
  const assetInstruments = useMemo(
    () => assetItems.filter((i) => i.to.startsWith("/asset?type=")),
    [assetItems],
  );
  ...
  const ALL_CATEGORIES: UnifiedCategory[] = useMemo(() => {
    return [
      ...
      {
        id: "ci-dashboard",
        title: "Dashboard Komoditas",
        group: "commodity_index",
        icon: Coins,
        getItems: () => {
          const item = commodityItems.find((i) => i.to === "/commodity-dashboard");
          return item ? [{ type: "app", item }] : [];
        },
      },
    ];
  ...
  // Filter categories by selected main tab and search
  const visibleCategories = useMemo(() => {
    ...
    return list;
  }, [mainTab, searchCategory, ALL_CATEGORIES]);
    ...
    <div className="w-full flex flex-col items-center">
      ...
      <div className="text-center mb-[calc(1.5rem+10pt)]">
        <h3 className="text-2xl sm:text-3xl tracking-tight flex items-center justify-center gap-1.5 text-foreground/90 font-normal">
          ...
          <span className="font-normal text-muted-foreground/80 mx-0.5">&</span>
      ...
      {/* Two-Column Section: Left Sidebar Categories + Right Grid */}
      <div className="w-full max-w-7xl flex flex-col md:flex-row gap-6 lg:gap-8 items-start">
        {/* Left Column: Subcategories list */}
        <div className="w-full md:w-64 lg:w-72 shrink-0 flex flex-col gap-2 p-3 sm:p-4 rounded-2xl bg-card/60 backdrop-blur-sm border border-border/50 shadow-xs">
          {/* Header Subkategori & Quick Search */}
          ...
          <div className="relative my-1">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-1.5 rounded-xl text-xs bg-background/80 border border-border/60 focus:outline-none focus:ring-1 focus:ring-primary text-foreground placeholder:text-muted-foreground"
            />
          </div>
        ...
        <div className="flex-1 w-full flex flex-col gap-6">
          {/* Breadcrumb Header */}
          <div className="flex items-center justify-between px-2 text-xs border-b border-border/40 pb-3">
            <div className="flex items-center gap-2 text-muted-foreground">
              <span>Financial Planning &amp; Wealth Management</span>
              <span>/</span>
              ...
              {activeCategoryTitle && (
                <>
                  <span>/</span>
                  <span className="text-foreground font-semibold">{activeCategoryTitle}</span>
                </>
              )}
            </div>
            <span className="text-muted-foreground text-[11px]">
              {displayedItems.length} modul ditemukan
            </span>
          </div>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-y-6 sm:gap-y-8 gap-x-3 sm:gap-x-4 place-items-center w-full">
