...
export function InteractionManagerApp() {
  ...
  const getSentimentBadge = (sentiment?: InteractionSentiment) => {
    switch (sentiment) {
      ...
      case "neutral":
        return (
          <span className="flex items-center gap-1 text-[11px] font-semibold text-slate-400 bg-slate-500/10 px-2 py-0.5 rounded-full border border-slate-500/20">
      ...
      <div className="flex-1 mt-4">
          ...
          <div className="space-y-6 max-w-4xl">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div className="p-5 bg-slate-800/80 border border-slate-700 rounded-xl">
                <div className="text-slate-400 text-xs font-semibold uppercase tracking-wider">Total Interaksi</div>
                <div className="text-3xl font-extrabold text-white mt-1">{stats.total}</div>
                <div className="text-xs text-slate-500 mt-1">Touchpoint tercatat</div>
              </div>

              <div className="p-5 bg-slate-800/80 border border-slate-700 rounded-xl">
                <div className="text-slate-400 text-xs font-semibold uppercase tracking-wider">Penyelesaian Follow-up</div>
                <div className="text-3xl font-extrabold text-emerald-400 mt-1">{stats.fuRate}%</div>
                <div className="text-xs text-slate-500 mt-1">{stats.doneFu} komitmen selesai</div>
              </div>

              <div className="p-5 bg-slate-800/80 border border-slate-700 rounded-xl">
                <div className="text-slate-400 text-xs font-semibold uppercase tracking-wider">Follow-up Terlambat</div>
                <div className="text-3xl font-extrabold text-rose-400 mt-1">{stats.overdueCount}</div>
                <div className="text-xs text-slate-500 mt-1">Melewati due date</div>
              </div>

              <div className="p-5 bg-slate-800/80 border border-slate-700 rounded-xl">
                <div className="text-slate-400 text-xs font-semibold uppercase tracking-wider">Relasi Dipantau</div>
                <div className="text-3xl font-extrabold text-sky-400 mt-1">{people.length}</div>
                <div className="text-xs text-slate-500 mt-1">Kontak People Manager</div>
              </div>
            </div>
          </div>
        )}
            ...
            <div className="space-y-3">
              {stats.peopleHealth.map(({ person, daysSince, lastDate, totalInteractions }) => {
                ...
                return (
                  <div
                    key={person.id}
                    className="p-4 bg-slate-800/80 border border-slate-700 rounded-xl flex items-center justify-between gap-4"
                  >
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-base text-slate-100">{person.fullName}</span>
                ...
                .map((fu) => {
                  ...
                  return (
                    ...
                    >
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-sm text-slate-100">{fu.description}</span>
                          {fu.dueDate < todayStr && (
                        ...
                        <div className="text-xs text-slate-400">
                          ...
                          <strong className="text-slate-200">{int?.personName}</strong>
                        </div>
                        {fu.linkedTaskId && (
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        {!fu.linkedTaskId && (
                          <button
                            ...
                            className="px-3 py-1.5 bg-slate-700 hover:bg-slate-600 text-cyan-300 border border-cyan-500/30 rounded-lg text-xs font-semibold flex items-center gap-1"
                          >
        ...
        {activeTab === "recent" && (
          <div className="space-y-4">
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-3 p-3 bg-slate-800/80 border border-slate-700 rounded-xl">
              <div className="relative w-full md:w-80">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-1.5 bg-slate-900 border border-slate-700 rounded-lg text-xs md:text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:border-sky-500"
                />
              </div>
              ...
              <div className="flex items-center gap-2 w-full md:w-auto justify-end flex-wrap">
                <select
                  ...
                  className="px-2.5 py-1.5 bg-slate-900 border border-slate-700 rounded-lg text-xs text-slate-300 focus:outline-none focus:border-sky-500"
                >
                ...
                {selectedPersonId && (
                  <button
                    ...
                    className="px-2.5 py-1.5 bg-slate-700 hover:bg-slate-600 text-slate-300 rounded-lg text-xs flex items-center gap-1"
                  >
            ...
            {/* TIMELINE FEED */}
            <div className="relative border-l-2 border-slate-700 pl-6 ml-4 space-y-6 py-2">
              {filteredInteractions.map((item) => {
                ...
                return (
                  <div key={item.id} className="relative group">
                    {/* Circle marker */}
                    <div className="absolute -left-[31px] top-1 size-4 rounded-full bg-sky-500 border-4 border-slate-900 shadow" />

                    <div className="p-5 bg-slate-800/80 border border-slate-700 rounded-xl space-y-3 hover:border-slate-600 transition">
                      <div className="flex items-start justify-between gap-4">
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            <span className="p-1 rounded bg-slate-700/60">{getTypeIcon(item.type)}</span>
                            <span className="font-bold text-base text-slate-100 hover:text-sky-400 cursor-pointer">
                              {item.personName}
                            </span>
                      ...
                      {/* Summary Note */}
                      <p className="text-xs md:text-sm text-slate-200 leading-relaxed bg-slate-900/60 p-3 rounded-lg border border-slate-800">
                        "{item.summary}"
                      </p>
                      ...
                      {item.outcome?.result && (
                        <div className="text-xs text-slate-300 space-y-1 pt-1">
                          ...
                          {item.outcome.nextSteps && (
                            <div>
                              <span className="font-semibold text-slate-400">Langkah Berikutnya:</span>{" "}
                              {item.outcome.nextSteps}
                            </div>
                          )}
                        </div>
                      ...
                      {itemFollowUps.length > 0 && (
                        <div className="pt-2 border-t border-slate-700/60 space-y-1.5">
                          ...
                          {itemFollowUps.map((fu) => (
                            ...
                            >
                              <div className="flex items-center gap-2">
                                <span className={fu.status === "done" ? "line-through text-slate-500" : "text-slate-200"}>
                                  {fu.description}
                                </span>
                                <span className="text-[10px] text-slate-400">(Batas: {fu.dueDate})</span>
                              </div>
                              <span
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-lg shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              ...
              <button onClick={() => setIsLogModalOpen(false)} className="text-slate-400 hover:text-slate-200">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleLogSubmit} className="space-y-3">
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-xs text-slate-100 focus:outline-none focus:border-sky-500"
                >
              ...
              <div className="grid grid-cols-3 gap-2">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-2.5 py-2 bg-slate-800 border border-slate-700 rounded-lg text-xs text-slate-100 focus:outline-none focus:border-sky-500"
                  >
                    <option value="call">Call / Telepon</option>
                    <option value="meeting">Meeting / Rapat</option>
                    <option value="email">Email</option>
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1">Arah</label>
                  <select
                    value={intDirection}
              ...
              <div className="grid grid-cols-2 gap-3 p-3 bg-slate-950 border border-slate-800 rounded-lg">
                <div>
                  <label className="block text-[11px] font-semibold text-slate-400 mb-1">Sentimen Hasil</label>
                  <select
                    value={intSentiment}
                ...
                <div>
                  <label className="block text-[11px] font-semibold text-slate-400 mb-1">Hasil / Kesimpulan</label>
                  <input
                    type="text"
              ...
              <div className="p-3 bg-slate-950 border border-slate-800 rounded-lg space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <input
                    ...
                    className="col-span-2 px-2.5 py-1.5 bg-slate-900 border border-slate-700 rounded text-xs text-slate-100"
                  />
                  <input
                    type="date"
                    value={fuDueDate}
