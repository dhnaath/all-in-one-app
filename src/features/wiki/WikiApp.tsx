  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] bg-slate-50 dark:bg-zinc-950 text-slate-800 dark:text-zinc-100 overflow-hidden font-sans">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-base font-bold text-slate-900 dark:text-zinc-100">Wiki Engine</h1>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-72 bg-white dark:bg-zinc-900 border-r border-slate-200 dark:border-zinc-800 flex flex-col shrink-0">
          <div className="p-3 border-b border-slate-200 dark:border-zinc-800">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-1.5 rounded-lg border border-slate-200 dark:border-zinc-700 bg-slate-50 dark:bg-zinc-800 text-xs outline-none focus:ring-1 focus:ring-blue-500"
              />
            </div>
          </div>
              ...
              orphanPages.length === 0 ? (
                ...
                orphanPages.map((p) => (
                  <button
                    ...
                    className="w-full text-left p-3 hover:bg-slate-50 space-y-1 text-xs"
                  >
                    <span className="text-[9px] px-1.5 py-0.2 rounded bg-amber-100 text-amber-800 font-bold uppercase">
                      Orphan
                    </span>
                    <h4 className="font-bold text-slate-900 dark:text-zinc-100">{p.title}</h4>
                  </button>
                ))
              )
            ) : activeView === "broken" ? (
              brokenLinks.length === 0 ? (
                ...
                brokenLinks.map((l) => (
                  <div key={l.id} className="p-3 space-y-1 text-xs">
                    ...
                    <p className="text-[10px] text-slate-400">Dirujuk dari halaman: {l.fromPageId}</p>
                    <button
        ...
        <div className="flex-1 bg-slate-50 dark:bg-zinc-950 overflow-y-auto flex flex-col">
          {activeView === "graph" ? (
            <div className="flex-1 p-6 flex flex-col">
              ...
              <div className="flex-1 rounded-2xl bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 p-6 flex flex-wrap gap-4 items-center justify-center overflow-auto shadow-2xs">
                {pages.map((p) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-[10px] font-bold text-slate-400 uppercase">Page</span>
                      </div>
                      <h4 className="text-xs font-bold text-slate-900 dark:text-zinc-100">
                        {p.title}
                      </h4>
                      {outLinks.length > 0 && (
                        <div className="mt-2 pt-2 border-t border-slate-100 dark:border-zinc-700 space-y-1">
                          {outLinks.map((l) => (
                            <div
                              key={l.id}
                ...
                {selectedPage.tags.length > 0 && (
                  <div className="flex items-center gap-1.5 flex-wrap">
                    {selectedPage.tags.map((t) => (
                      <span
                        key={t}
                        className="text-[10px] px-2 py-0.5 rounded-md bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-zinc-300"
                      >
                        #{t}
                      </span>
                    ))}
                    ...
                    {activeBacklinks.map((bl) => {
                      ...
                      return (
                        <div
                          ...
                          className="p-3 rounded-xl border border-slate-100 dark:border-zinc-800 bg-slate-50/50 dark:bg-zinc-800/40 hover:bg-slate-100 cursor-pointer text-xs space-y-0.5"
                        >
          ...
          <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl w-full max-w-md p-6 space-y-4">
            ...
            <div className="space-y-2 text-xs">
              <div className="flex justify-between py-1 border-b border-slate-100 dark:border-zinc-800">
                <span className="text-slate-500">Halaman Paling Banyak Ditautkan (Hub):</span>
                <span className="font-bold">[[{stats.topTitle}]]</span>
              </div>
              <div className="flex justify-between py-1 border-b border-slate-100 dark:border-zinc-800">
                <span className="text-slate-500">Orphan Pages (Tanpa Tautan):</span>
                <span className="font-bold text-amber-600">{stats.orphanCount} Halaman</span>
              </div>
              <div className="flex justify-between py-1">
                <span className="text-slate-500">Pending / Broken Links:</span>
