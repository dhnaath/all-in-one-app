...
export const Route = createFileRoute("/tows")({
  head: () => ({
    meta: [
      { title: "TOWS Matrix — Client OS" },
      { name: "description", content: "Matriks Strategi Taktis & Panduan Eksekusi Lengkap TOWS Matrix." },
    ],
  }),
  component: TOWSPage,
});
...
function TOWSPage() {
  const towsData: FrameworkContent = getFrameworkData("TOWS Matrix");
  ...
  const [worksheetNotes, setWorksheetNotes] = useState<Record<string, string>>(() => {
    try {
      const saved = localStorage.getItem("bizcoach_worksheet_TOWS Matrix");
  ...
  const [checkedTasks, setCheckedTasks] = useState<Record<number, boolean>>(() => {
    try {
      const saved = localStorage.getItem("bizcoach_checklist_TOWS Matrix");
      return saved ? JSON.parse(saved) : {};
    } catch {
      return {};
    }
  ...
  const handleNoteChange = (bagianKey: string, value: string) => {
    ...
    try {
      localStorage.setItem("bizcoach_worksheet_TOWS Matrix", JSON.stringify(updated));
    } catch {}
  };
  ...
  const toggleChecklist = (index: number) => {
    ...
    try {
      localStorage.setItem("bizcoach_checklist_TOWS Matrix", JSON.stringify(updated));
    } catch {}
  };

  const resetWorksheet = () => {
    if (window.confirm("Hapus semua catatan lembar kerja TOWS Matrix?")) {
      setWorksheetNotes({});
      localStorage.removeItem("bizcoach_worksheet_TOWS Matrix");
  ...
  const resetChecklist = () => {
    setCheckedTasks({});
    localStorage.removeItem("bizcoach_checklist_TOWS Matrix");
    triggerNotify("Checklist action plan di-reset!");
  };

  const handleCopySummary = () => {
    const text = `TOWS MATRIX - Strategic Management
Teori: ${towsData.teori.deskripsi}
Manfaat: ${towsData.teori.manfaat}

Draft Area:
${towsData.draft.map(d => `- ${d.bagian}: ${d.hint}`).join('\n')}

Tutorial:
${towsData.tutorial.map((t, idx) => `${idx + 1}. ${t.step}: ${t.desc}`).join('\n')}

Action Plan:
${towsData.actionPlan.map(a => `[ ] ${a}`).join('\n')}`;

    navigator.clipboard.writeText(text);
    triggerNotify("Ringkasan TOWS Matrix disalin ke clipboard!");
  };

  const handleDownloadWorksheet = () => {
    const lines = [
      `# LEMBAR KERJA BISNIS: TOWS MATRIX`,
      `Kategori: Strategic Management`,
      `Tanggal: ${new Date().toLocaleDateString('id-ID')}`,
      `\n## Catatan Rencana Kerja:`,
      ...towsData.draft.map(d => {
        const val = worksheetNotes[d.bagian] || "(Belum diisi)";
        return `\n### ${d.bagian}\n*Panduan: ${d.hint}*\n**Catatan:**\n${val}\n`;
      }),
      `\n## Action Plan Terpilih:`,
      ...towsData.actionPlan.map((a, i) => `${checkedTasks[i] ? '[x]' : '[ ]'} ${a}`)
    ];
    ...
    a.href = url;
    a.download = `TOWS_Matrix_Worksheet.md`;
    a.click();
    URL.revokeObjectURL(url);
    triggerNotify("File lembar kerja diunduh!");
  };

  const totalChecklist = towsData.actionPlan.length;
  const completedChecklist = towsData.actionPlan.filter((_, idx) => checkedTasks[idx]).length;
  const percentComplete = totalChecklist > 0 ? Math.round((completedChecklist / totalChecklist) * 100) : 0;

  return (
    <AppShell 
      title="TOWS Matrix" 
      subtitle="Matriks Strategi Taktis & Panduan Eksekusi Lengkap"
      actions={
    ...
    >
      ...
      <div className="w-full space-y-10">
        ...
        <section>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <Panel>
              ...
              <p className="text-muted-foreground leading-relaxed text-sm">
                {towsData.teori.manfaat}
              </p>
            </Panel>
          </div>
        </section>

        {/* Bagian 2: Desain & Layout Visual (UI) */}
        <section>
          <SectionHeader number="2" title="Desain & Layout Visual (UI)" icon={LayoutTemplate} />
          <div className="flex items-center justify-between mb-4">
            <p className="text-muted-foreground text-sm">
              Representasi visual standar industri (<span className="font-semibold text-foreground">{towsData.layout.tipe}</span>):
            </p>
            <span className="text-xs font-semibold px-2.5 py-1 bg-muted text-muted-foreground rounded-md border border-border">
              {towsData.layout.elemen.length} Komponen Kuadran
            </span>
          </div>
          <VisualLayoutMockup layout={towsData.layout} />
        </section>

        {/* Bagian 3: Draft Konten & Lembar Kerja Interaktif */}
        <section>
        ...
        {/* Bagian 5: Action Plan & Interactive Checklist */}
        <section>
          <div className="bg-foreground rounded-2xl p-6 md:p-8 text-background shadow-lg relative overflow-hidden">
            ...
            <div className="space-y-2 relative z-10">
              {towsData.actionPlan.map((plan, i) => {
